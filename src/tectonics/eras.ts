import PALEO_LAND from './paleoLand.generated.json'
...
export interface Era {
  ...
  land: ReadonlyArray<ReadonlyArray<readonly [number, number]>>
}
...
const ERAS_RAW: ReadonlyArray<Omit<Era, 'land'>> = [
  {
    id: 'pangaea',
    name: 'Pangaea',
    mya: 250,
...
/** Eras with their per-era paleoshoreline land merged in from the generated
 *  PaleoDEM data (keyed by era id). */
export const ERAS: ReadonlyArray<Era> = ERAS_RAW.map((e) => ({
  ...e,
  land: (PALEO_LAND as unknown as Record<string, Era['land']>)[e.id] ?? [],
}))
